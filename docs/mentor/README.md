# Mentor Guide Index

Use this folder when you are facilitating teams, checking safety, or reviewing
student demos.

## Start Here

1. [Mentor Guide](./mentor-guide.md) - delivery style, session rhythm,
   check-ins, debugging protocol, AI limits, and facilitation notes.
2. [Curriculum Content Source](./curriculum-source.md) - what content Soma uses
   and what boundaries mentors should keep clear.
3. [Demo Rubric](./rubric.md) - minimum MVP bar and suggested awards.

## Run The Workshop

- [6-Session Runbook](../workshop/sessions/README.md) - live session plan.
- [Workshop Course](../workshop/README.md) - deeper lesson path, lab path, and
  source standard.
- [Lesson Index](../workshop/lessons/README.md) - the 12 concept lessons.
- [Concept To Code Map](../workshop/concept-to-code-map.md) - links sessions,
  lessons, code files, labs, and AI coding prompts.
- [Student Guide](../student/README.md) - what students see.

## Review Safety

- [Safety Checklist](../api-safety-checklist.md) - key, data, AI honesty, and
  Debug Lab checks.
- [`/api/coach` Contract](../api-coach-contract.md) - request and response
  shape.
- [Testing And Debugging](../testing-debugging.md) - local test and manual
  browser gates.
- [Gemini Key Setup](../gemini-key-setup.md) - how to add a real provider key
  safely.
- [Deploy To Vercel](../deploy-vercel.md) - public deployment walkthrough.

Back to [Docs Home](../README.md).
