...
function buildCoachResult(payload) {
  ...
  if (triggerText.includes("quota-test")) {
    return {
      status: 429,
      payload: { error: "The classroom AI quota or rate limit was reached. Try again later or ask a mentor." }
    };
  }
  ...
  if (triggerText.includes("network-test")) {
    return {
      status: 503,
      payload: { error: "The AI coach is temporarily unavailable. The app should show this honestly." }
    };
  }
